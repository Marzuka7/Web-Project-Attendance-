...
function  getSessionHTML(rv)
{
    ...
    {
        let cs=rv[i];
        let formattedTerm = formatTermNumber(cs['term']); // Helper function to format the term number
        x = x + `<option value=${cs['id']}>${cs['year']} Series ${formattedTerm} Semester</option>`;
    }
    return x;
}

// x=x+`<option value=${cs['id']}>${"Series - "+cs['year']+" Semester - "+cs['term']}</option>`;
function formatTermNumber(term) {
    if (term === 1) return "1st";
    if (term === 2) return "2nd";
    if (term === 3) return "3rd";
    return term + "th"; // For all other cases
}
